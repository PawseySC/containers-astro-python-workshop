...
To get started, let's comment on a simple example, where we're using a *Ubuntu* image and we want to add some developer tools using `apt`: some build tools, *git* and *wget8.  We're going to use it to practice with advice 1 to 3 above.  The corresponding files are in the directory `2.ubuntu/`.
...
We're going to use this image again in the next session.
